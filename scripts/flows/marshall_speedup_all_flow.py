...
import logging
logger = logging.getLogger("marshall_speedup_all_flow")
...
def verify_title_active(win: WindowsScreenshotHelper, debug: bool = False) -> bool:
    ...
    if debug:
        logger.info(f"    Title active icon: found={found}, score={score:.4f}")
    return found
...
def apply_marshall_and_verify(
    ...
    if debug:
        logger.info("  Step 1: Going to marked Royal City...")
    success = go_to_mark_flow(adb, win, debug=debug)
    if not success:
        logger.info("  ERROR: Failed to go to marked location")
        return False
    ...
    if debug:
        logger.info("  Step 2: Applying Marshall title...")
    success = title_management_flow(adb, "marshall", screenshot_helper=win, debug=debug)
    if not success:
        logger.info("  ERROR: Failed to apply Marshall title")
        return_to_base_view(adb, win, debug=debug)
        return False

    # Step 3: Return to base view
    if debug:
        logger.info("  Step 3: Returning to base view...")
    return_to_base_view(adb, win, debug=debug)
    time.sleep(0.5)

    # Step 4: Verify title is active
    if debug:
        logger.info("  Step 4: Verifying title is active...")
    ...
    while time.time() - start < POLL_TIMEOUT:
        if verify_title_active(win, debug=debug):
            if debug:
                logger.info("  Marshall title VERIFIED active!")
            return True
        time.sleep(POLL_INTERVAL)

    logger.info("  WARNING: Could not verify title is active (icon not found)")
...
def speedup_all_training_barracks(
    ...
    # First, go to town
    if debug:
        logger.info("  Going to TOWN view...")
    go_to_town(adb, debug=debug)
    time.sleep(0.5)
    ...
    if debug:
        logger.info(f"  Found {len(training_barracks)} TRAINING barracks: {[f'B{i+1}' for i in training_barracks]}")

    if not training_barracks:
        logger.info("  No TRAINING barracks found")
        return 0
    ...
    for idx in training_barracks:
        ...
        if success:
            speedup_count += 1
            if debug:
                logger.info(f"  Barrack {idx + 1} speedup SUCCESS")
        else:
            logger.info(f"  Barrack {idx + 1} speedup FAILED")
...
def marshall_speedup_all_flow(
    ...
    try:
        ...
        if not skip_marshall:
            logger.info("=== Applying Marshall Title ===")
            result['marshall_applied'] = apply_marshall_and_verify(adb, win, debug=debug)
            if not result['marshall_applied']:
                logger.info("WARNING: Marshall application may have failed, continuing anyway...")
        else:
            logger.info("=== Skipping Marshall (--skip-marshall) ===")
...
if __name__ == "__main__":
    ...
    logger.info("=== Marshall Speedup All Flow ===")
    ...
    logger.info(f"\n=== Results ===")
    logger.info(f"Marshall applied: {result['marshall_applied']}")
    logger.info(f"Barracks sped up: {result['barracks_sped_up']}")
