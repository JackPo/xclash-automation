"""
Smart zoom setup using template matching for World/Town button detection
1. Detect World/Town button via template matching
...
def detect_button_template():
    """Detect World/Town button using blue color detection"""
    screenshot = capture_screenshot()
    img = cv2.imread(screenshot)
    if img is None:
        return False, "Cannot load screenshot"

    height, width = img.shape[:2]

    # Crop lower-right corner where button should be
    x1, y1 = max(0, 2200), max(0, 1200)
    x2, y2 = width, height
    corner = img[y1:y2, x1:x2]

    # Convert to HSV and look for blue button
    hsv = cv2.cvtColor(corner, cv2.COLOR_BGR2HSV)

    # Blue color range for the button
    ...
    # If we have significant blue pixels, button is visible
    if blue_pixels > 1000:  # At least 1000 blue pixels
        return True, f"Button detected ({blue_pixels} blue pixels)"
    else:
        return False, f"Button not found (only {blue_pixels} blue pixels)"
...
def setup_optimal_zoom():
    ...
    print("="*60)
    print("SMART ZOOM SETUP (Template Matching)")
    print("="*60)

    # Step 1: Ensure World/Town button is visible
    print("\n[1/5] Detecting World/Town button...")
    detected, message = detect_button_template()
    print(f"  {message}")
    ...
    # Step 4: Start at baseline (no zoom adjustment)
    # Baseline appears to be close to optimal already
    print("\n[4/5] Starting at baseline zoom (no adjustment)...")
    time.sleep(1)
