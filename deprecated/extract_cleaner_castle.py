...
if image is None:
    ...
    # Bounding box for the new, cleaner castle
    # Format: (x_min, y_min, x_max, y_max)
    bounding_box = (1400, 450, 1600, 650)

    # Crop the image
    template = image[bounding_box[1]:bounding_box[3], bounding_box[0]:bounding_box[2]]

    # Save the template, overwriting the previous one
    template_path = f'templates/complex_castle_1.png'
    cv2.imwrite(template_path, template)
    print(f"Saved new template to {template_path}")
