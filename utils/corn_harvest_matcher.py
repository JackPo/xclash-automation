...
Template extracted from 4K screenshot at coordinates (1102, 930) with size 99x74.

FIXED specs (4K resolution):
- Extraction position: (1102, 930)
- Size: 99x74 pixels
- Click position: (1151, 967) - center of template
"""
...
class CornHarvestMatcher:
    ...
    def __init__(
        ...
        if template_path is None:
            template_path = base_dir / "templates" / "ground_truth" / "corn_harvest_bubble_4k.png"
    ...
    def _save_debug_crop(self, roi: np.ndarray, score: float) -> None:
        ...
        try:
            if roi.size == 0:
                return
            debug_path = self.debug_dir / f"corn_present_{score:.3f}.png"
