...
FLOW_CONFIGS = {
    ...
    "desert_python_rally": {"cooldown": 60, "idle_required": 0},  # python rally mode: opted in; fires ON SIGHT (no idle gate) like sandstorm - rally the moment the cobra icon appears in WORLD
    ...
    "quick_production": {"cooldown": 68400, "idle_required": IDLE_THRESHOLD},  # re-check after 19h: the blind 24h
    # assumption drifted from the GAME cooldown (QP sat ready for hours while the daemon waited until hour 24).
    # The flow OCRs the REAL cooldown before acting and records the true remaining time when it skips - so an
    # early re-check costs one read-only panel visit and the OCR is the authority, not the stopwatch.
}
...
class DaemonScheduler:
    ...
    # =========================================================================
    # ASSIST ALLY MODE - when on, assist ally castles showing the helmet marker
    # (click castle under helmet -> Assist) whenever one appears in WORLD view.
    # =========================================================================

    @_locked
    def get_assist_mode(self) -> tuple[bool, datetime | None]:
        """Get assist-ally mode status. Returns (active, expires)."""
        state = self.schedule.get("assist_mode", {})
        active = state.get("active", False)
        expires_str = state.get("expires")
        if not active:
            return False, None
        if expires_str:
            try:
                ...
                if datetime.now(timezone.utc) > expires:
                    self.clear_assist_mode()
                    logger.info("Assist mode expired")
                    return False, None
                return True, expires
            except (ValueError, TypeError):
                return False, None
    ...
    @_locked
    def set_assist_mode(self, hours: float | None = None) -> datetime | None:
        """Enable assist-ally mode for N hours (None = until manually stopped)."""
        expires = None
        if hours:
            expires = datetime.now(timezone.utc) + timedelta(hours=hours)
            self.schedule["assist_mode"] = {
                "active": True,
                "expires": expires.isoformat(),
                "set_at": datetime.now(timezone.utc).isoformat(),
            }
        ...
        self.save()
        logger.info(f"Assist mode enabled (expires: {expires})")
        return expires

    @_locked
    def clear_assist_mode(self) -> None:
        """Clear assist-ally mode."""
